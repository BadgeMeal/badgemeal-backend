...
public interface DrawRepository extends JpaRepository<Draw, Long> {
    ...
    @Query("SELECT new hack.badgemeal.apis.common.dto.DrawUserDto(u.address, d.drawId, d.count, d.drawInfo.round) " +
            "FROM Draw d INNER JOIN hack.badgemeal.apis.domain.draw.model.User u ON u.address = :address AND d.user.address = :address WHERE d.drawInfo.isNow = 'Y'")
    DrawUserDto getDrawNowRoundByAddress(String address);
}
